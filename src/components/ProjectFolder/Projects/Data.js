const ProjectData = [
	{
		id: "meditation",
		dark: false,
		topline: "Meditation App",
		headline: "App that helps you meditate",
		description:
			"An app with different natural soothing sound that helps boost your productivity.",
		buttonLabel: "Let's Meditate",
		imgStart: false,
		imgsrc: require("../../../content/svg/svg-1.svg").default,
		alt: "Car",
		primary: true,
		route: "/projects/meditation",
	},
  	// {
	// 	id: "to-do-app",
	// 	dark: true,
	// 	topline: "To Do App",
	// 	headline: "Organize your day with the todo app.",
	// 	description:
	// 		"A productivity based app that keeps track of your to do list.",
	// 	buttonLabel: "Let's Do it!",
	// 	imgStart: true,
	// 	imgsrc: require("../../../content/svg/svg-3.svg").default,
	// 	alt: "To Do",
	// 	primary: true,
    // 	route: '/projects',
	// },
	// {
	// 	id: "GeoQuiz",
	// 	dark: true,
	// 	topline: "Geoquiz App",
	// 	headline: "App that helps you improve geography knowledge",
	// 	description:
	// 		"Lorem ipsum dolor sit amet consectetur, adipisicing elit. Magni odio maiores iusto! Ut porro doloribus illum fugiat dolores rerum magni sit molestiae vero provident perspiciatis qui enim nihil id repudiandae commodi ea sed beatae error eos, quasi non quo accusantium! Ullam laudantium reprehenderit neque fugiat debitis aspernatur quibusdam harum nisi.",
	// 	buttonLabel: "Do Geoquiz",
	// 	imgStart: true,
	// 	imgsrc: require("../../../content/svg/svg-2.svg").default,
	// 	alt: "Geo",
	// 	primary: true,
	// 	route: "/projects",
	// },
	// {
	// 	id: "Chess",
	// 	dark: false,
	// 	topline: "Chess",
	// 	headline: "Play chess with me",
	// 	description:
	// 		"Lorem ipsum dolor sit amet consectetur, adipisicing elit. Magni odio maiores iusto! Ut porro doloribus illum fugiat dolores rerum magni sit molestiae vero provident perspiciatis qui enim nihil id repudiandae commodi ea sed beatae error eos, quasi non quo accusantium! Ullam laudantium reprehenderit neque fugiat debitis aspernatur quibusdam harum nisi.",
	// 	buttonLabel: "Play Chess",
	// 	imgStart: false,
	// 	imgsrc: require("../../../content/svg/svg-2.svg").default,
	// 	alt: "chess",
	// 	primary: true,
    // route: '/projects'
	// },
	// {
	// 	id: "Sudoku",
	// 	dark: true,
	// 	topline: "Sudoku",
	// 	headline: "Play Sudoku with me.",
	// 	description:
	// 		"Lorem ipsum dolor sit amet consectetur, adipisicing elit. Magni odio maiores iusto! Ut porro doloribus illum fugiat dolores rerum magni sit molestiae vero provident perspiciatis qui enim nihil id repudiandae commodi ea sed beatae error eos, quasi non quo accusantium! Ullam laudantium reprehenderit neque fugiat debitis aspernatur quibusdam harum nisi.",
	// 	buttonLabel: "Play Sudoku",
	// 	imgStart: true,
	// 	imgsrc: require("../../../content/svg/svg-3.svg").default,
	// 	alt: "Play Sudoku",
	// 	primary: true,
    // route: '/projects'
	// },
];

export default ProjectData;
